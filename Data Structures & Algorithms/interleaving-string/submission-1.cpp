...
public:
    bool isInterleave(string s1, string s2, string s3) {
        vector<vector<vector<int>>> dp(s1.size()+2,vector<vector<int>>(s2.size()+2,vector<int>(s3.size()+2,0)));
        dp[s1.size()][s2.size()][s3.size()]=1;
        for(int i=s1.size();i>=0 ;i--){
            for(int j=s2.size();j>=0;j--){
                for(int k=s3.size();k>=0;k--){
                    if(i<s1.size() and k<s3.size() and s1[i]==s3[k] and dp[i+1][j][k+1]) dp[i][j][k]=true;
                    if(j<s2.size() and k<s3.size() and s2[j]==s3[k] and dp[i][j+1][k+1]) dp[i][j][k]=true;
                }
            }
        }
        return dp[0][0][0];
